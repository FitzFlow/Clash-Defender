...
void shoot_barb_lv1(all_t *all)
{
    ...
    while (all->tab_of_site[line] != NULL) {
        while (all->tab_of_barbare[line2] != NULL) {
            distance = func_dist(all->tab_of_barbare[line2]->position.x
            + 38, all->tab_of_barbare[line2]->position.y + 49,
            all->tab_of_site[line]->position.x + 68,
            all->tab_of_site[line]->position.y + 62);
            resize_barb_lv1(all, distance, line, line2);
            line2++;
        }
        line2 = 0;
        line++;
